...
class Chapter:
    ...
    def get_page(self):
        if self.url.startswith('http'):
            ...
            return urlopen(Request('https://wanderinginn.com/', headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/601.3.9 (KHTML, like Gecko) Version/9.0.2 Safari/601.3.9'}), timeout=5)
        ...
        for img in contents.find_all('img'):
            ...
            if img_filename:
                with open(os.path.join(image_path, img_filename), 'wb') as fo:
                    fo.write(urlopen(Request(img['src'], headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/601.3.9 (KHTML, like Gecko) Version/9.0.2 Safari/601.3.9'}), timeout=10, context=ssl.create_default_context()).read())
